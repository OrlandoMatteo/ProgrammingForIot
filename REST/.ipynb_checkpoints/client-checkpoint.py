...
class Client(object):
	...
	def getHistory(self):
		...
		if x=='D':
			...
			r=requests.get('https://api.exchangeratesapi.io/{}-{}-{}'.format(year,month,day))
			print (json.dumps(r.json(),indent=4))
		if x=='I':
			...
			r=requests.get('https://api.exchangeratesapi.io/history?start_at={}-{}-{}&end_at={}-{}-{}'.format(s_year,s_month,s_day,e_year,e_month,e_day))
			print (json.dumps(r.json(),indent=4))
